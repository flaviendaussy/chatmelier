...
export async function getRecentActivityResource() {
    if (!supabase)
    ...
    if (addedError)
        throw new Error(`Error fetching additions: ${addedError.message}`);
    // Recent consumptions
    ...
    if (consumedError)
        throw new Error(`Error fetching consumptions: ${consumedError.message}`);
