...
export async function getBottleDetails(args) {
    if (!supabase)
    ...
    `)
        ...
        .single();
    if (bottleError)
        throw new Error(`Bottle not found: ${bottleError.message}`);
    ...
    if (bottle.wine.cuvee_parcel)
        output += `Cuvée / Parcel: ${bottle.wine.cuvee_parcel}\n`;
    ...
    if (bottle.wine.appellation)
        output += `Appellation: ${bottle.wine.appellation}\n`;
    ...
    if (tastings.length > 0) {
        ...
        tastings.forEach(t => {
            ...
            if (t.rating)
                output += `Rating: ${t.rating}/5 ⭐\n`;
            if (t.occasion)
                output += `Occasion: ${t.occasion}\n`;
            if (t.food_paired)
                output += `Paired with: ${t.food_paired}\n`;
            if (t.tasting_notes || t.notes)
                output += `Notes: ${t.tasting_notes || t.notes}\n`;
            output += `---\n`;
        });
    }
    else {
