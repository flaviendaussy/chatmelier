...
export async function addBottle(args: z.infer<typeof AddBottleInput>) {
  ...
  if (existingWines && existingWines.length > 0) {
    ...
    if (createError) throw new Error(`Failed to create wine: ${createError.message}`);
    wineId = newWine.id;
  }
  ...
  if (bottleError) throw new Error(`Failed to add bottle: ${bottleError.message}`);
