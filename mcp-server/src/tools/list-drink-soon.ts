...
export async function listDrinkSoon(args: z.infer<typeof ListDrinkSoonInput>) {
  ...
  const drinkSoon = bottles.filter(b => {
    if (!b.wine.vintage) return false;
    const age = currentYear - b.wine.vintage;
    const type = (b.wine.type || '').toLowerCase();
    
    if (type.includes('white') || type.includes('rose')) return age >= 3;
    if (type.includes('red')) return age >= 7;
    return false;
