...
export async function searchBottles(args: z.infer<typeof SearchBottlesInput>) {
  ...
  let results = data as unknown as BottleWithWine[];
  ...
  if (args.query) {
    ...
    results = results.filter(b => 
      b.wine.name.toLowerCase().includes(q) ||
  ...
  return results.map(b => 
    `- [${b.id}] ${b.wine.vintage || 'NV'} ${b.wine.producer || ''} ${b.wine.name} (${b.wine.type || 'Unknown'} - ${b.wine.region || 'Unknown'}, ${b.wine.country || 'Unknown'}) | Qty: ${b.quantity}`
  ).join('\n');
}
